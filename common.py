...
class Options(object):
  def __init__(self):
    ...
    self.temp_dir = None      # using specified temp_dir instead of '/tmp'      12/12/2016@zhuangyan


OPTIONS = Options()


# add for OTA remote signing    10/13/2015@chenyuqin
OPTIONS.signer_server = 'compiler213'
OPTIONS.signer_base = ''

# add for oem project           04/19/2016@chenyuqin
OPTIONS.dict_oem_project = os.getenv('BBK_OEM_PROJECTS', None)
OPTIONS.dict_oem_dir = []

# add for updating block-based full OTA package in multi-thread         04/18/2016@chenyuqin
OPTIONS.system_blockimg_pieces = 16

# add for adjusting number of online processors         09/27/2016@chenyuqin
OPTIONS.cpu_online_map = None           # default no limitations
OPTIONS.CPU_ONLINE_MAX = 16

OPTIONS.verify_blocks_type = 'MD5'
...
def LoadInfoDict(input_file):
  ...
  # add for oem properties          04/19/2016@chenyuqin
...
def LoadBuildProp(read_helper):
  try:
    ...
    data = ""
  try:
    data += read_helper("META/oem.prop")
  except KeyError:
    print "Warning: could not find META/oem.prop in %s" % zip
...
def UnzipTemp(filename, pattern=None):
  ...
  tmp = tempfile.mkdtemp(prefix="targetfiles-", dir=OPTIONS.temp_dir)
...
def ParseOptions(argv,
  ...
  try:
    opts, args = getopt.getopt(
        ...
        ["help", "verbose", "path=", "signapk_path=", "extra_signapk_args=",
         ...
         "extra=", "signer_server=", "signer_base=", "system_blockimg_pieces=", "cpu_online_map=", "temp_dir="] +
  ...
  for o, a in opts:
    ...
    elif o in ("--signer_server",):
      OPTIONS.signer_server = a
    elif o in ("--signer_base",):
      OPTIONS.signer_base = a
    elif o in ("--system_blockimg_pieces",):
      if a.isdigit():
        OPTIONS.system_blockimg_pieces = int(a)
      else:
        raise ValueError("Cannot parse value %r for option %r - only "
                         "integers are allowed." % (a, o))
      if OPTIONS.system_blockimg_pieces > 16:
        OPTIONS.system_blockimg_pieces = 16         # max pieces number
        print ('warning: adjust OPTIONS.system_blockimg_pieces to 16')
    elif o in ("--cpu_online_map",):
      cpu_online_map = ('b' * OPTIONS.CPU_ONLINE_MAX)                   # max processor number
      pieces = a.split(',')
      for piece in pieces:
        start = None
        end = None
        try:
          if piece.count('-') == 0:
            ...
            print ('Skip invalid parsed value %s for cpu_online_map' % (piece,))
            continue
        except ValueError, e:
          print ('Cannot parse value %s for cpu_online_map' % (piece,))
          sys.exit(1)
        if start < 0 or start >= OPTIONS.CPU_ONLINE_MAX or end < 0 or end >= OPTIONS.CPU_ONLINE_MAX:
          print ('Skip invalid parsed value %s for cpu_online_map' % (piece,))
          continue
        while start <= end:
          cpu_online_map = cpu_online_map[:start] + 'w' + cpu_online_map[start+1:]
          start += 1
      if 'w' in cpu_online_map:
        OPTIONS.cpu_online_map = cpu_online_map
    elif o in ("--temp_dir",):
      OPTIONS.temp_dir = a
...
class BlockDifference(object):
  def __init__(self, partition, tgt, src=None, check_first_block=False,
    ...
    if partition == 'system' and self.src == None:
      b.Compute(self.path, OPTIONS.system_blockimg_pieces)
    else:
      b.Compute(self.path)

    # add for oem.zip (as no "fstab" in OPTIONS.info_dict when making oem.zip)              06/12/2016@chenyuqin
    if partition.startswith('oem') or partition.startswith('OEM'):
      self.device = '/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/cust'
    else:
  ...
  def WriteScript(self, script, output_zip, progress=None):
    ...
    if progress:
      script.ShowProgress(progress, progress * 100)
    if self.partition == 'system' and self.src == None:
      self._WriteUpdate(script, output_zip, OPTIONS.system_blockimg_pieces)
    else:
      self._WriteUpdate(script, output_zip)
    # self._WritePostInstallVerifyScript(script)            skip verification temporarily    06/02/2016@chenyuqin

  def SplitRangesIntoPieces(self, ranges, pieces_number):
    range_pieces = []
    max_blocks_per_piece = ranges.size() / pieces_number
    while (ranges.size() > max_blocks_per_piece):
      range_first = ranges.first(max_blocks_per_piece)
      range_pieces.append(range_first)
      ranges = ranges.subtract(range_first)
    # Handle remaining blocks. (one more piece)
    if ranges.size():
      range_pieces.append(ranges)
    return range_pieces
  ...
  def WriteVerifyScript(self, script):
    ...
    if not self.src:
      ...
      options = ''
      if OPTIONS.verify_blocks_type == 'MD5':
        options += 'mddddd,'
      ranges = self.src.care_map.subtract(self.src.clobbered_blocks)
      ranges_str = ranges.to_string_raw()
      if self.version >= 3:
        if partition == 'system':
          range_pieces = self.SplitRangesIntoPieces(ranges, 32)          # FIXED to 32 pieces for 'system'
          range_sha1_check_str = ', '.join([('"%s", "%s"') % (piece.to_string_raw(), self.HashBlocks(self.src, piece)) for piece in range_pieces])
          script.AppendExtra(('if (range_sha1_check("%s", %s) || '
                              'block_image_verify("%s", '
                              'package_extract_file("%s.transfer.list"), '
                              '"%s.new.dat", "%s.patch.dat", "%s")) then') % (
                              self.device, range_sha1_check_str,
  ...
  def _WriteUpdate(self, script, output_zip, system_blockimg_pieces=1):
    ZipWrite(output_zip,
             '{}.transfer.list'.format(self.path),
             '{}.transfer.list'.format(self.partition))
    if os.path.isfile('{}.transfer.list.origin'.format(self.path)):
      ZipWrite(output_zip,
               '{}.transfer.list.origin'.format(self.path),
               '{}.transfer.list.origin'.format(self.partition))
    ZipWrite(output_zip,
             '{}.new.dat'.format(self.path),
             '{}.new.dat'.format(self.partition))
    if self.partition == 'system' and system_blockimg_pieces > 1:           # partition not necessary
      index = 1
      while index < system_blockimg_pieces:
    ...
    call = ('block_image_update("{device}", '
            ...
            '"{partition}.new.dat", "{partition}.patch.dat", "{options}");\n'.format(
                device=self.device, partition=self.partition, options=options))
...
# added for updating survival partition, basically copied from MakeRecoveryPatch (nevertheless we could still use the variable recovery_img)            06/24/2016@chenyuqin
...
def MakeSurvivalPatch(input_dir, output_sink, survival_img, boot_img,
  ...
  if os.path.exists(path):
    diff_program.append("-b")
    diff_program.append(path)
    bonus_args = "-b /system/etc/recovery-resource.dat"         # loki: recovery.img and survival.img share the same recovery-resource.dat (res/)
  else:
    bonus_args = ""
  ...
  try:
    boot_type, boot_device = GetTypeAndDevice("/boot", info_dict)
    survival_type, survival_device = GetTypeAndDevice("/survival", info_dict)           # NOTICE: 'survival' is not actually a mount point
  except KeyError:
    return

  sh = """#!/system/bin/sh
if ! applypatch -c %(survival_type)s:%(survival_device)s:%(survival_size)d:%(survival_sha1)s; then
  applypatch %(bonus_args)s %(boot_type)s:%(boot_device)s:%(boot_size)d:%(boot_sha1)s %(survival_type)s:%(survival_device)s %(survival_sha1)s %(survival_size)d %(boot_sha1)s:/system/wwh && log -t recovery "Installing new ww image: succeeded" || log -t recovery "Installing new ww image: failed"
else
